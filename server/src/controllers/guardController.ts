...
export const createGuard = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { guardCode, name, phone, email, gender, permanentLocationId, permanentShift, weeklyOff, status } = req.body;
    ...
    const guard = await prisma.guard.create({
      data: {
        ...
        email,
        gender,
        permanentLocationId: permanentLocationId ? Number(permanentLocationId) : null,
        permanentShift: permanentShift || null,
        weeklyOff,
